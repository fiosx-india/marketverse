...
st.markdown("---")
st.subheader("🔍 Fully Updated Smart Section & Logic Analyzer")
...
def fully_updated_analyze(content):
    report = "=== REFINED SMART DIAGNOSTIC REPORT ===\n\n"
    lines = content.split('\n')
    
    sections_found = 0
    real_issues = 0
    
    # நீங்கள் குறிப்பிட்ட அனைத்துச் செயல்பாடுகளையும் உள்ளடக்கிய பட்டியல்
    valid_elements = [
    ...
    while i < len(lines):
        ...
        # செக்ஷன், ஹெட்டிங் அல்லது ஃபங்ஷனைக் கண்டறிதல்
        if line.startswith('def ') or 'st.subheader' in line or 'st.markdown' in line:
            # அனலைசரின் சொந்தக் குறியீடுகளைத் தவிர்ப்பதற்கான பாதுகாப்புச் சரிபார்ப்பு
            if 'smart_analyze_sections' in line or 'fully_updated_analyze' in line:
                i += 1
                continue
            ...
            # அருகில் உள்ள அடுத்த 8 வரிகளைச் சோதித்துப் பார்த்தல்
            next_lines_chunk = "\n".join(lines[i:min(i+9, len(lines))])
            
            # அனுமதிக்கப்பட்ட அனைத்து UI மற்றும் லாஜிக் கூறுகள் உள்ளதா எனச் சரிபார்த்தல்
            has_valid_logic = any(keyword in next_lines_chunk for keyword in valid_elements)
            ...
            if is_pure_separator:
                report += f"  ℹ️ நிலை: வெறும் செப்பரேட்டர் (Separator).\n"
            elif has_valid_logic:
                report += f"  ✅ நிலை: சரியாக இணைக்கப்பட்டுள்ளது (சரியான UI/Logic உள்ளது).\n"
            else:
                real_issues += 1
                report += f"  ❌ உண்மையான குறைபாடு: அருகில் எந்த UI அல்லது லாஜிக்கும் இல்லை!\n"
            
            report += "-" * 50 + "\n"
        i += 1
        
    report += f"\n[இறுதி முடிவு]: அனைத்து புதிய விதிகளின்படியும் கண்டறியப்பட்ட உண்மையான குறைபாடுகள்: {real_issues}\n"
    return report

# பகுப்பாய்வு ரிப்போர்ட்டை உருவாக்குதல்
final_report = fully_updated_analyze(code_content)

# திரையில் காட்டுவது
st.text_area("Refined Diagnostic Report:", final_report, height=250)

# காப்பி செய்யும் வசதி
if st.button("Copy Refined Report"):
    st.code(final_report, language="text")
    st.success("Refined report copied successfully for your session only!")
